...
function displayCartItems() {
    ...
    // Sanitize cart data: ensure price and quantity are numbers, fallback to 0/1 if invalid
    let hasCorruptItem = false;
    cart = cart.map(item => {
        let price = parseFloat(item.price);
        let quantity = parseInt(item.quantity);
        if (isNaN(price)) price = 0;
        if (isNaN(quantity) || quantity < 1) quantity = 1;
        if (price === 0 || !item.id || !item.name) hasCorruptItem = true;
        return { ...item, price, quantity };
    });
    if (hasCorruptItem) {
        showCartUpdateToast('Error: Some cart items are invalid and have been reset.');
        // Optionally, remove corrupt items:
        cart = cart.filter(item => item.price > 0 && item.id && item.name);
        localStorage.setItem('cart', JSON.stringify(cart));
    }
...
function updateCartSummary() {
    let cart = JSON.parse(localStorage.getItem('cart')) || [];
    // Sanitize cart data for summary
    cart = cart.map(item => {
        let price = parseFloat(item.price);
        let quantity = parseInt(item.quantity);
        if (isNaN(price)) price = 0;
    ...
    document.getElementById('subtotal').textContent = `₱${isNaN(subtotal) ? '0.00' : formatPriceWithCommas(subtotal)}`;
    document.getElementById('shipping').textContent = shipping === 0 ? 'FREE' : `₱${formatPriceWithCommas(shipping)}`;
    document.getElementById('total-price').textContent = `₱${isNaN(total) ? '0.00' : formatPriceWithCommas(total)}`;
